...
def main(argv):
    pos_path = pathlib.Path(sys.argv[1])
    pos_predictions = []
    pos_incorrect = []
    for filename in pos_path.iterdir():
        if filename.is_file() and filename.suffix == '.txt':
            with open(filename, 'r', encoding='utf8') as f:
                text = f.read()
                clean_text = cleanFileContents(text)
                tokens_with_counts = countTokens(clean_text)
                prediction = predictSimplistic(tokens_with_counts)
                if prediction == "NEGATIVE" or prediction == "NONE":
                    pos_incorrect.append(filename)
                pos_predictions.append(prediction)

    neg_path = pathlib.Path(sys.argv[2])
    neg_predictions = []
    neg_incorrect = []
    for filename in neg_path.iterdir():
        if filename.is_file() and filename.suffix == '.txt':
            with open(filename, 'r', encoding='utf8') as f:
                text = f.read()
                clean_text = cleanFileContents(text)
                tokens_with_counts = countTokens(clean_text)
                prediction = predictSimplistic(tokens_with_counts)
                if prediction == "POSITIVE" or prediction == "NONE":
                    neg_incorrect.append(filename)
                neg_predictions.append(prediction)

    pos_correct = pos_predictions.count("POSITIVE")
    
    neg_correct = neg_predictions.count("NEGATIVE")

    total_accuracy = (pos_correct + neg_correct) / (len(pos_predictions) + len(neg_predictions))

    # overall accuracy
    print(round(total_accuracy, 4))
    pos_tp = pos_predictions.count("POSITIVE")
    pos_fn = len(pos_predictions) - pos_tp
    pos_fp = neg_predictions.count("POSITIVE")
    # positive precision
    print(round(pos_tp / (pos_tp + pos_fp), 4))
    # positve recall
    print(round(pos_tp / (pos_tp + pos_fn), 4))

    neg_tp = neg_predictions.count("NEGATIVE")
    neg_fn = len(neg_predictions) - neg_tp
    neg_fp = pos_predictions.count("NEGATIVE")
    # negative precision
    print(round(neg_tp / (neg_tp + neg_fp), 4))
    # negative recall
    print(round(neg_tp / (neg_tp + neg_fn), 4))
